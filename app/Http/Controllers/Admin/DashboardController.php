...
class DashboardController extends Controller
{
    ...
    public function index()
    {
          ...
          $totals = User::selectRaw("count(case when status = 'active' then 1 end) as active")
           ...
           // Count of active products which don't belong to any user.
            ...
            //Summarized price of all active attached products (from the previous subpoint if prod1 price is 100$, prod2 price is 120$, prod3 price is 200$, the summarized price will be 3 x 100 + 9 x 120 = 1380).
             ...
             ->join('products','products.id','=','user_product.product_id')->where('products.status','active')->sum(DB::raw('products.price * user_product.qty'));


            // Summarized prices of all active products per user. For example - John Summer - 85$, Lennon Green - 107$.

              $userSummarizedPrice = UserProduct::select('products.price','products.title','products.id as product_id','user_product.qty','products.status','users.id as user_id','users.name',DB::raw('products.price * user_product.qty as totalAmount'))
      ...
      return view('admin.dashboard', compact('totals','CountactiveProduct','CountactiveVerifiedAttachUser','CountactiveProduct_withoutAttach','AmountactiveAttachedProduct','PriceActiveAttachedProduct','userSummarizedPrice'));
    }
